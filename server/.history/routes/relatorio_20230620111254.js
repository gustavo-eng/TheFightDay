...
router.get('/', async (req, res) => {
      ...
      <body>
        <div style="display: flex; justify-content: center; align-items: center; height: 100vh;">
          <div style="background-color: white; padding: 20px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1);">
            ...
            <a href="/download" download="chart.pdf" style="display: block; text-align: center;">Download PDF</a>
          </div>
        </div>
      </body>
    </html>
  ...
  // Envie a página normal com o PDF como resposta
  res.contentType('text/html');
  res.send(page.content());
});

router.get('/download', async (req, res) => {
  // Gere o PDF novamente para fazer o download
  const browser = await puppeteer.launch();
  const page = await browser.newPage();
  await page.setContent(`
    <html>
