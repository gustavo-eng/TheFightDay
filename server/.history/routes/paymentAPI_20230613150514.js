...
router.post('/', controllAcces.accessControl,(req, res) => {
    const { email, name, nameCompetition, categoryWeight, categoryYear, picture, idCompeticao } = req.body
    ...
    paymentDAO.save(email, name, nameCompetition, categoryWeight, categoryYear, picture, idCompeticao)
    .then(payment => {
        res.status(200).json(payment)
