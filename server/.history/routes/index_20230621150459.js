...
var TaskValidator = require('../validator/TaskValidator')

var validateLogin = require('../validator/ValidateLogin')
...
router.post('/contato', TaskValidator.validateFields ,async (req, res) => {
  ...
  console.warn('entrou em  router.post(/contato), TaskValidator.validateFields')
  ...
  transport.verify( (error, succes) => {
    if(error) {
      ...
      console.log(' -------- Server is ready to take your emails OK EMAIL OK ------------ ')
    }
  ...
  const mailOptions = {
      ...
      to: process.env.email_origin, // Substitua com o email do destinatário
      subject: subject,
      text: `Nome: ${name}\nEmail: ${email}\nAssunto: ${subject}\nMensagem: ${message}`
  };
   ...
   transport.sendMail(mailOptions, (error, data) => {
    ...
    if(error) {
      ...
      document.getElementById('error-message').innerHTML = 'Ocorreu um erro ao enviar a mensagem. Por favor, tente novamente mais tarde.';
