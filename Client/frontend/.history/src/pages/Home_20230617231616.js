...
const Home = () => {
    ...
    // const handlePayment = async (idCompetition) => {
    //     // evt.preventDefault()
    //     setCardPayment(false)
    //     console.log('Entrou no handleSubmitt')
    //     const response = await controllServicePayment.savePayment(idCompetition, token, email, user, nameCompetition, categoryWeight, categoryYear,  picture  )
    ...
    const handlePayment = async (idCompetition) => {
        setCardPayment(false);
        console.log('Entrou no handleSubmitt');
        const competitionChosen = competition.find((comp) => comp._id === idCompetition);
        if (competitionChosen) {
                   ...
                   {cardPayment ? null : <button onClick={modalPayment}> Pagar </button>}
                    ...
                    {cardPayment ?
                        <>
                            ...
                            <button onClick={() => handlePayment(comp._id)}>Pagar2!!!!!!</button>
                            <button onClick={() => setCardPayment(!cardPayment)}> Cancelar Pagamento </button>
                        </>
                    : null}
                </div>
           ))
