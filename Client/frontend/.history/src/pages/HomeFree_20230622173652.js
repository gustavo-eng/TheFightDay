            ...
            <h1 className="h1_pagination"> Competições </h1>
            {
                competition.length > 0 ?
                 competition.map((comp, index) => {
                        return (
                        <div className="container_competition">
                            <h3 className="title">{comp.nome}</h3>
                            <h3 className="price">Preço: {comp.valor}  reais </h3>
                            <h2 className="data_payment"> Data Pagamento {comp.DataPagamento}</h2>
                            <h2 className="data_comp"> Data Competição  {comp.DataCompeticao}</h2>

                        </div>

                        )
                })
                : <div className="missing">Sem registros </div>

            }
            <button className="btn-next" onClick={(e)=> handleNextPage(e) }> Próximo</button>
            <button className="btn-before" onClick={(e) => handleBeforePage(e)}> Voltar </button>
        </div>
    )
