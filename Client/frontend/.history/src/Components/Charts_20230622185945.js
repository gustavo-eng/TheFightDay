...
const ChartComponent = () => {
    ...
    const [qtdCompetition, setQtdCompetition] = useState(0)
    // const a = []
    // a.length
     useEffect(() => {
        ...
        .then(data => {
            console.log('data ====> ');
            console.log(data);
            setQtdCompetition(data.length)
            console.log('QUANTIDADE COMPETICAO')
            console.log(qtdCompetition)
        })
    ...
    const options= {
        ...
        xaxis: {
          categories: ['Usuários', 'Pagamentos ','Competições Pagas']
        }
    }
      const series= [{
        name: 'series-1',
        data: [10, qtdPayments, qtdCompetition]
      }]
